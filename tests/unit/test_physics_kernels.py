...
from trichroma.engine.physics_reference import (
    BULK_ABSORB,
    BULK_NONE,
    BULK_SCATTER,
...
def test_chroma_cross_preserves_positive_zero_on_exact_cancellation():
    ...
    from trichroma.engine.physics_kernels import cross_chroma_fast
...
def test_triton_photon_source_is_deterministic_tile_invariant_and_isotropic():
    ...
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required")
    from trichroma.engine.physics_kernels import generate_photon_bomb
...
def test_triton_optical_primitives_match_cpu_references():
    ...
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required")
    from trichroma.engine.physics_kernels import (
