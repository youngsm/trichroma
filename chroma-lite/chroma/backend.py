...
* ``cuda`` (default): the original PyCUDA implementation.
* ``triton``: the Triton implementation in :mod:`chroma.triton.compat`,
  which does not import PyCUDA.
...
def backend_name(environ=None):
    ...
    value = environ.get("CHROMA_BACKEND", "cuda").strip().lower() or "cuda"
