...
def nearest_hit_kernel(
    ...
    LEAF: tl.constexpr, WIRE_MODE: tl.constexpr, BLOCK: tl.constexpr, STATS: tl.constexpr = False,
    stats_ptr=None,
):
...
def _wire_plane(go, wx, wy, wz, dx, dy, dz, dn, wn0, uux, uuy, uuz, vvx, vvy, vvz, nnx, nny, nnz,
    ...
    """Installed-Chroma FP32 wire intersection for one plane (lanes in ``go``)."""
...
@triton.jit
def _all_wires(valid, ox, oy, oz, dx, dy, dz, cap, wires_ptr, n_wires):
