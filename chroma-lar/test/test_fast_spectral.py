...
def pipeline():
    ...
    calibration = OpticalCalibration.load(Path(__file__).parents[1]/"benchmarks/optical_validation/full_detector_synthetic_calibration.json")
...
def test_gpu_scintillation_matches_cpu_source(simulation, seed):
    ...
    s._source(state, n, (-1000, 0, 0), 0, seed, first, 17, 29.)
    positions = state.pos.cpu().numpy()
    expected = s.calibration.source.photons(positions, seed=seed, photon_id_base=first, event_indices=17, times=29.)
    for index, value, atol in ((1, expected.direction, 9.e-7), (2, expected.polarization, 1.2e-6),
                               (3, expected.times, .003), (10, expected.wavelengths, 2.e-5)):
        np.testing.assert_allclose(state[index].cpu().numpy(), value, rtol=3.e-7, atol=atol)
    ...
    assert np.max(abs((state.direction*state.polarization).sum(1).cpu().numpy())) < 5.e-7
...
def test_bulk_epoch_commits_only_certified_collisions(simulation, varying):
    ...
    absorption, scattering = s.model.properties.absorption_length.clone(), s.model.properties.scattering_length.clone()
    if varying:
        state.wavelengths.copy_(torch.linspace(120, 500, n, device="cuda"))
        absorption[s.lar_index].copy_(torch.linspace(1000, 100000, s.model.optics.wavelength_grid.count, device="cuda"))
        scattering[s.lar_index].copy_(torch.linspace(100, 2000, s.model.optics.wavelength_grid.count, device="cuda"))
    initial = [a.cpu().numpy().copy() for a in state]
    q, out, boundary = [DeviceQueue.allocate(n) for _ in range(3)]
    q.buffer.copy_(torch.arange(n, dtype=torch.int32, device="cuda")); q.count.fill_(n)
    g = s.model.optics.wavelength_grid
    bulk_epoch[(triton.cdiv(n, 128),)](*state, q.buffer, q.count, n, out.buffer, out.count,
        boundary.buffer, boundary.count, absorption, scattering, s.model.properties.group_velocity, seed, 200,
        *map(float, s.query.safe_lower), *map(float, s.query.safe_upper), float(g.start), float(g.step),
        LAR=s.lar_index, NW=g.count, HISTORY=1, BLOCK=128, enable_fp_fusion=False)
    ...
    with np.errstate(divide="ignore"):
        distances = np.where(direction > 0, (s.query.safe_upper-position)/direction,
                            np.where(direction < 0, (s.query.safe_lower-position)/direction, np.inf))
    exit_distance = distances.min(1)
    safe = np.maximum(0, exit_distance-np.maximum(.01, 2.e-6*abs(exit_distance)))
    collision = np.minimum(da, ds) < safe
    np.testing.assert_array_equal(final[9], collision.astype(np.int32))
    np.testing.assert_array_equal(np.sort(boundary.tensor().cpu().numpy()), np.flatnonzero(~collision))
    ...
    expected = rayleigh_scatter(initial[1][scatter], initial[2][scatter],
                               *(uniform(ids, seed, i)[scatter] for i in range(2, 6)))
    np.testing.assert_allclose(final[1][scatter], expected[0], atol=2.e-6, rtol=1.e-6)
    np.testing.assert_allclose(final[2][scatter], expected[1], atol=2.e-6, rtol=1.e-6)
    np.testing.assert_allclose(final[0][collision], initial[0][collision]+np.minimum(da, ds)[collision, None]*initial[1][collision],
                               rtol=3.e-6, atol=.0005)
    velocity = np.interp(initial[10], s.calibration.wavelengths, s.model.properties.group_velocity[s.lar_index].cpu().numpy())
    np.testing.assert_allclose(final[3][collision], (initial[3]+np.minimum(da, ds)/velocity)[collision], rtol=3.e-7, atol=.003)
...
def test_wls_boundary_retains_spectrum_time_and_material_side(simulation):
    ...
    state = s._empty_state(n)
    s._source(state, n, (-1000, 0, 0), 0, seed, 0, 0, 0)
    state.direction.copy_(torch.tensor([0., 0., 1.], device="cuda"))
    state.polarization.copy_(torch.tensor([1., 0., 0.], device="cuda"))
    state.times.zero_(); state.wavelengths.fill_(128)
    ...
    normals = torch.tensor([0., 0., -1.], device="cuda").repeat(n, 1)
    hit = (full(1., torch.float32), normals, full(s.lar_index), full(glass), full(sid), full(0), full(tri), full(-1))
    arrays = list(s.model.properties)
    arrays[1] = torch.full_like(arrays[1], 1.e30)
    arrays[2] = torch.full_like(arrays[2], 1.e30)
    g = s.model.optics.wavelength_grid
    boundary_step[(triton.cdiv(n, 128),)](*state, q.buffer, q.count, n, output.buffer, output.count,
        *hit, s.query.scene_device["pmt_scene_material1_index"], *arrays, seed, 200,
        float(g.start), float(g.step), NW=g.count, BLOCK=128, enable_fp_fusion=False)
    flags, wavelength, time, direction = (state[i].cpu().numpy() for i in (4, 10, 3, 1))
    ids = np.arange(n, dtype=np.int64)
    reemit_probability = np.interp(128, s.calibration.wavelengths, s.model.optics.surfaces.reemit[sid])
    ...
    np.testing.assert_allclose(wavelength[expected], cdf.sample(uniform(ids, seed, 8))[expected], atol=4.e-5, rtol=1.e-7)
    delay = s.model.surfaces[sid].reemission_time_cdf.sample(uniform(ids, seed, 9))
    velocity = np.interp(128, s.calibration.wavelengths, s.model.properties.group_velocity[s.lar_index].cpu().numpy())
    np.testing.assert_allclose(time[expected], (1/velocity+delay)[expected], atol=3.e-5, rtol=2.e-6)
...
@pytest.mark.parametrize("noise,adc_bits", [(0., None), (.4, None), (.4, 12)])
def test_gpu_waveforms_match_fractional_overlapping_cpu_pulses(noise, adc_bits):
    ...
    pe = Photoelectrons(times, rng.uniform(.2, 1.8, n), rng.integers(0, 5, n), np.arange(n),
                        rng.choice([11, 37], n), times)
    options = dict(event_indices=[37, 99, 11], channel_count=5, start_ns=-20.123,
        sample_period_ns=.3, sample_count=600, pulse_times_ns=[0., .7, 3.1, 6.2, 12.3],
        pulse_adc_per_pe=[0., 2., 1.3, .4, 0.], baseline=100., noise_rms=noise, adc_bits=adc_bits, seed=991)
    ...
    if adc_bits:
        ...
        np.testing.assert_allclose(cpu.samples, gpu.samples, atol=2.e-11, rtol=1.e-13)
...
def test_full_fast_pipeline_preserves_empty_event_and_wls(simulation, pipeline):
    result = pipeline.simulate_voxel(100000, event_id=11, event_indices=[11, 99], seed=1729, keep_final_states=True)
    ...
    np.testing.assert_array_equal(result.waveforms.samples[1], simulation.calibration.digitizer["baseline"])
    from chroma.triton.optical_response import digitize
    cpu = digitize(result.photoelectrons, event_indices=[11, 99], channel_count=162, seed=1729, **simulation.calibration.digitizer)
...
def test_gpu_deposition_counts_and_zero_light_event(simulation, pipeline):
    ...
    energies = [.03, .02, 0.]
    options = dict(event_indices=[11, 37, 99], times=[3., 5., 7.], seed=872)
    ...
    np.testing.assert_array_equal(result.waveforms.samples[2], simulation.calibration.digitizer["baseline"])
    empty = pipeline.simulate_depositions([[-1000, 0, 0]], [0.], event_indices=[99])
    assert empty.transport.photon_count == 0 and len(empty.photoelectrons.times) == 0
    np.testing.assert_array_equal(empty.waveforms.samples, simulation.calibration.digitizer["baseline"])
...
def test_spectral_reachability_checks_every_wavelength(simulation):
    ...
    for field in ("absorb", "detect", "reflect_diffuse"):
        barrier.set(field, 0.)
    barrier.set("reflect_specular", [.5, 1, 1], [120, 450, 500])
...
def test_source_outside_compiled_component_rejected(simulation):
    ...
    p = Photons([[1000., 0, 0]], [[1., 0, 0]], [[0., 1., 0]], [450.])
...
def test_lossless_detector_with_absorbing_steel_has_no_false_bulk_crossings(simulation):
    ...
    c.surfaces["reflect00"].set("reflect_diffuse", 0.)
    c.surfaces["reflect00"].set("absorb", 1.)
    full = SpectralDetectorSimulation(c)
    result = full.simulate_voxel(10000, voxel_size=0., seed=11, max_steps=2048, keep_final_states=True)
    ...
    assert np.all(np.any(np.isclose(abs(position), 3240., atol=.002, rtol=0), axis=1))
...
def test_separated_pmt_bounds_preserve_complete_geometry_query(simulation):
    ...
    original = nearest_pmt_hit(replace(accelerator, coarse_bounds_min=None, coarse_bounds_max=None,
                                      coarse_box_count=1), origin, rays, ray_tile=None)
    for name in ("distances", "triangle_ids", "instance_ids", "channel_ids", "world_normals"):
        np.testing.assert_array_equal(getattr(separated, name).cpu().numpy(), getattr(original, name).cpu().numpy())
