...
This is the README of the original Chroma (via
[youngsm/chroma-lite](https://github.com/youngsm/chroma-lite)), kept with its
documentation (`doc/`), images and container recipes (`installation/`, for
the original CUDA backend). Chroma's code is `src/chroma` in this repository;
see the [TriChroma README](../../README.md) for the Triton backend.
