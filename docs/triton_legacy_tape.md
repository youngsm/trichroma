# Bitwise legacy mode: native RNG tapes and exact arithmetic

This is the reference for the bitwise legacy mode of the Triton drop-in
...
`chroma/sim_cuda.py` installs `chroma.triton.legacy.record_cuda` when
...
Reader (`chroma.triton.legacy.tape`, NumPy only; device views need torch):
...
batch.device_inputs(device)           # chroma.triton.engine.api.DevicePhotons
...
## 6. Exact-math Triton library (`chroma/triton/engine/exact.py`)
...
(`_complex_ptx.py`, generated by `chroma/triton/legacy/tools/ptx_extract.py`).
